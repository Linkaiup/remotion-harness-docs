...
| 渲染速度 | CPU多线程渲染+GPU渲染提高性能 / Lambda 并行 | ~41 秒/5 秒视频 | ~41 秒/5 秒视频 |
...
|---|---|---|---|---|
| 服务端 `@remotion/renderer` | Headless Chrome 打开 Composition，逐帧截图 | CSS 支持完整，稳定性高 | 有服务器成本，渲染速度可CPU多线程渲染+GPU渲染提高性能 | MVP 最终导出和兜底渲染 |
...
| 速度 | CPU多线程渲染+GPU渲染提高性能/ 可分片并行 | 0.5-2 fps，取决于设备 |
...
└───────────────┬──────────────────────────────┬─────────────┘
  ...
  └─────────────┬────────┘  └──────────────────┬───────────┘
                │                              │
                │ Draft Patch                  │ Rendered Asset
                └──────────────┬───────────────┘
                               ▼
┌────────────────────────────────────────────────────────────┐
│           Merge → Validate → Preview → Repair → Commit     │
│  Schema / Timeline / Asset / Font / Visual / Performance   │
└─────────────────────────────┬──────────────────────────────┘
                              ▼
                    ┌─────────────────┐
                    │  统一导出管线     │
...
| **净节省** | **~¥190K-233K/年** |
| 开发投入（15 个 MVP 模块） | 6-8 周 × 1-3 开发者 |
...
| 渲染速度慢（可用CPU多线程渲染+GPU渲染提高性能，但需要考虑用户并发） | 中 | Lambda 分布式；帧缓存 + 增量渲染 |
...
| 方案 | 许可证 | 优势 | 劣势 |
|---|---|---|---|
| **HyperFrames**（HeyGen 开源） | Apache 2.0 | 完全免费、AI 生成 HTML 更可靠，但生成动画效果待验证 | 不支持浏览器端渲染、开源仅 4 个月 |
